...
1. **Seed Storage**  
   ...
   While vulnerabilities like the "Unbindable Kemmy Schmidt" attack {{BIND}} pose risks in non-HSM environments, where expanded private keys can be manipulated, this is significantly mitigated when using an HSM. In an HSM, the private key is protected, and manipulation of the private key material is prevented, making such attacks much harder to execute. To mitigate risks in non-HSM environments, the honest decryptor should re-derive their private key from the seed at decryption time. This ensures consistency and eliminates the potential for manipulation of expanded private keys, especially when keys are not securely stored.
   
   It is crucial that the **HSM must store only the seed** and not the private key itself, as this approach minimizes risk and ensures compliance with security standards. The ML-DSA and ML-KEM private key formats, as specified in {{?I-D.ietf-lamps-dilithium-certificates}} and {{?I-D.ietf-lamps-kyber-certificates}}, represent the private key using a seed from which the expanded private key is derived. The expanded private key format in ML-KEM only provides LEAK-BIND-K-PK and LEAK-BIND-K-CT security but does not ensure MAL-BIND-K-CT or MAL-BIND-K-PK security. Using the 64-byte seed format strengthens binding security by adding MAL-BIND-K-CT security, though MAL-BIND-K-PK remains unaddressed (Section 4 of {{!I-D.sfluhrer-cfrg-ml-kem-security-considerations}}).
   ...
   However, private key import poses a challenge if the system is designed to store only seeds. When a user attempts to import an already expanded private key, there is a mismatch between the key format used internally (seed-based) and the expanded private key. This issue arises because the internal format is designed for efficient key generation and storage by deriving the private key from the seed, while the expanded private key is already fully computed. As NIST has not defined a single private key format for PQC algorithms, this creates a potential gap in interoperability.
