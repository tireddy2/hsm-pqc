...
In constrained environments, devices are often assumed to be clients initiating outbound connections. However,
many devices also act as servers, enforce local authentication policies, or verify digital signatures on
firmware, certificates, or commands. These use cases demand inbound authentication capabilities and significantly
intersect with the challenges of adopting post-quantum cryptography (PQC).
...
* Larger Signatures and Certificate Sizes
   ...
   Post-quantum signature schemes typically produce much larger public keys and signatures than their classical
...
* Increased RAM usage and performance profile.
   ...
   Post-quantum signature verification often demands significantly more RAM than classical schemes. For example,
   ML-DSA-65 in its high-performance configuration may require over 68 KB of memory during signing and up to 10
   KB during verification on Cortex-M4-class devices.

   This poses challenges for use cases such as firmware verification (e.g., secure boot) and certificate validation
   during TLS handshakes or local attestation.
...
or LMS instead of SLH-DSA). These choices should be aligned with the device’s operational profile, available memory, and
